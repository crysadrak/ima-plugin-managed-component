...
export default class AbstractManagedComponent extends AbstractComponent {
  ...
  constructor(props, context) {
    if (!context || !context.$Utils || !context.$Utils.$Dispatcher) {
      throw new Error("The React context needs to be provided, it must " +
          "contain the view utils in the $Utils property, and the view " +
          "utils must contain the IMA event dispatcher in the $Dispatcher " +
          "property")
    }
    ...
    if ($Debug) {
      if (
        ...
        throw new TypeError("The componentWillUnmount method is final and " +
            "cannot be overridden, use the onBeforeUnmount method instead")
      }
    }
    ...
    /**
     * Map of registered callbacks executed periodically to the native interval
     * IDs.
     *
     * @type {Map<function(), number>}
     */
   ...
   *
   * The listener will be automatically deregistered right before the component
   * will be unmounted from the DOM (by the {@linkcode componentWillUnmount()}
   * method).
   *
  ...
  /**
   * Registers the provided callback to be executed after the specified delay.
   *
   * The callback will be automatically deregistered right before the component
   * will be unmounted from the DOM (by the {@linkcode componentWillUnmount()}
   * method).
   *
  ...
  setTimeout(callback, delay) {
    if (this[PRIVATE.pendingTimeouts].has(callback)) {
      throw new Error("The provided callback is already pending to be " +
          "executed. If you need to reschedule the callback execution, " +
          "clear the callback's scheduled timeout execution using the " +
   ...
   * execution.
   *
   * @param {function()} callback The callback that is scheduled for execution.
   */
  ...
  setInterval(callback, period) {
    if (this[PRIVATE.activeIntervals].has(callback)) {
      throw new Error("The provided callback is already registered to be " +
          "executed periodically. If you need to reschedule the callback " +
          "execution, clear the callback's scheduled interval execution " +
          "using the clearInterval() method first")
    }
  ...
  /**
   * Executes the {@linkcode onBeforeUnmount}, then deregisters all registered
   * event bus, dispatcher and DOM event listeners, all pending timeouts and
   * active intervals registered by this component.
   *
   * @override
   */
  componentWillUnmount() {
    ...
    for (let eventTarget of this[PRIVATE.eventBusListeners].keys()) {
      let eventListeners = this[PRIVATE.eventBusListeners].get(eventTarget)
      for (let eventName of eventListeners.keys()) {
        for (let realListener of eventListeners.get(eventName).values()) {
          super.unlisten(eventTarget, eventName, realListener)
        }
      }
    }
    ...
    for (let eventName of this[PRIVATE.dispatcherListeners].keys()) {
      for (let listener of this[PRIVATE.dispatcherListeners].get(eventName)) {
        this.utils.$Dispatcher.unlisten(eventName, listener, this)
      }
    }
